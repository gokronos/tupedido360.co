...
import { ArrowLeft, Banknote, CheckCircle2, Clock3, MapPin, PackageCheck, Phone, RefreshCw, ShoppingBag, Trash2, Truck, UtensilsCrossed } from "lucide-react";
...
function OrderCard({ order, busy, canManagePayment, onAction }: { order: Order; busy: boolean; canManagePayment:boolean;onAction: (payload: Record<string, unknown>) => void }) {
  ...
  return <article className={`order-card status-${order.status}`}>
    ...
    <div className={`order-payment-box ${order.paid?"is-paid":"is-pending"}`}><div><Banknote size={20}/><span><strong>{order.paid?"Pedido pagado":"Pago pendiente"}</strong><small>{order.paid?"El pago fue registrado por el restaurante.":"Todavía no se ha registrado el pago."}</small></span></div>{canManagePayment&&<button disabled={busy} onClick={()=>onAction({action:"togglePaid",id:order.id})}>{busy?"Actualizando...":order.paid?"Marcar como pendiente":"Marcar como pagado"}</button>}</div>
    <footer>{order.paymentMethod==="transfer"&&canManagePayment&&<label><span>Validación</span><select value={order.paymentStatus} disabled={busy} onChange={(event) => onAction({ action: "updatePaymentStatus", id: order.id, status: event.target.value })}><option value="pending">Pendiente</option><option value="pending_verification">Por verificar</option><option value="verified">Pagado</option></select></label>}<label><span>Estado</span><select value={order.status} disabled={busy} onChange={(event) => onAction({ action: "updateStatus", id: order.id, status: event.target.value })}>{statusOptions.map((status) => <option value={status} key={status}>{statusInfo[status].label}</option>)}</select></label><button className="delete-order-button" disabled={busy} title="Eliminar pedido" onClick={()=>{const reason=window.prompt(`¿Por qué se elimina el pedido ${order.reference}?`);if(reason!==null)onAction({action:"deleteOrder",id:order.id,reason})}}><Trash2 size={16}/>Eliminar</button></footer>
  </article>;
}
