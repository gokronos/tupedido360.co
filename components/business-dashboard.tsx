...
export function BusinessDashboard({
  ...
  const visibleNavigation = navigation.filter(
    (item) =>
      (allowed[session.role ?? ""] ?? []).includes(item.id) &&
      (!playApp || item.id !== "subscription"),
  );
    ...
    <main className={`dashboard-shell${playApp ? " play-app-dashboard" : ""}`}>
      <aside className="dashboard-sidebar">
        <div className="dashboard-brand">
          ...
          <div>
            TuPedido360<small>{session.businessName}</small>
          </div>
        </div>
        <nav>
      ...
      <section className="dashboard-main">
        <div className="mobile-brand-bar">
          <div className="dashboard-brand" style={{ padding: 0, margin: 0 }}>
            ...
            <div>
              TuPedido360
              <small style={{ color: "#b8cbc2", fontSize: "11px" }}>
        ...
        {section === "summary" && (
          <Summary
            ...
            onSettings={() => setSection("subscription")}
            playApp={playApp}
          />
        )}
        {section === "orders" && (
        ...
        {section === "design" && (
          <DesignManager slug={session.businessSlug ?? ""} />
        )}
        ...
        {![
          ...
          <PendingSection
            name={
              navigation.find((item) => item.id === section)?.label ?? "Sección"
            }
          />
        )}
      </section>
    ...
    <>
          ...
          <span>
            ...
            <small>
              {data?.subscription?.isLifetime
                ...
                : data?.subscription?.status === "trialing"
                  ? `La prueba gratuita finaliza el ${new Date(data.subscription.trialEndsAt).toLocaleDateString("es-CO")}`
                  : playApp
                    ? "Consulte aquí el estado de su cuenta."
                    : `Plan de ${money(data?.subscription?.monthlyPriceCop ?? 30000)} al mes`}
            </small>
          </span>
        </div>
        {!playApp && <button onClick={onSettings}>Ver suscripción</button>}
      </div>
      <div className="panel-section-title">
        <div>
          <h2>Resumen de hoy</h2>
          <p>La actividad de tu negocio aparecerá aquí.</p>
        </div>
        <a
          href={`https://${session.businessSlug}.tupedido360.co`}
          target="_blank"
          rel="noreferrer"
        >
          Ver tienda <ExternalLink size={16} />
        </a>
      </div>
      <div className="metric-grid">
        <article>
          ...
          <small>Recibidos hoy</small>
        </article>
        <article>
          <span>Ventas</span>
          <strong>{metrics ? money(metrics.salesToday) : "..."}</strong>
        ...
        <article>
          ...
          <small>Disponibles en el menú</small>
        </article>
      </div>
      {metrics?.activeProducts === 0 && (
        <section className="empty-orders">
          ...
          <p>Comienza agregando los productos que ofrecerá tu negocio.</p>
          <button onClick={onProducts}>Crear primer producto</button>
        </section>
      )}
    </>
  );
...
function PendingSection({ name }: { name: string }) {
  return (
    <section className="empty-orders">
      <Store size={30} />
      <h3>{name}</h3>
      <p>Esta sección será el siguiente módulo que construiremos.</p>
    </section>
  );
}
