...
export function LoginForm({ expectedSlug }: { expectedSlug?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({})) as { error?: string; destination?: string };
  ...
  return (
    <form className="login-form" onSubmit={submit}>
      <label><span>Usuario o correo</span><input name="login" type="text" autoComplete="username" required placeholder="Su usuario o correo" /></label>
      <label>
        <span>Contraseña</span>
        <div className="password-input">
          <input name="password" type={showPassword ? "text" : "password"} autoComplete="current-password" required placeholder="Su contraseña" />
