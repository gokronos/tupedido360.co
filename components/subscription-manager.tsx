    ...
    <div className="subscription-manager">
      ...
      {!data.isLifetime && (
        <div className="renewal-section">
          <div className="renewal-heading">
            <h3>
              <Zap size={20} /> Planes de Renovación Disponibles
            </h3>
            <p>
          ...
          {payError && <p className="form-error renewal-error">{payError}</p>}
          <div className="renewal-plans">
            {plans.map((plan) => (
              <article key={plan.id} className={plan.months === 12 ? "featured" : ""}>
                <div>
                  <div className="renewal-plan-head">
                    <span>
                      {plan.badge}
                    </span>
                    {plan.months === 12 && <Sparkles size={16} />}
                  </div>
                  <strong>{plan.name}</strong>
                  <b>{money(plan.priceCop)}</b>
                  {plan.discount && <small>{plan.discount}</small>}
                </div>
                <button
          ...
          <p className="subscription-support renewal-support">
      ...
      {checkoutUrl && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 999999, backgroundColor: "rgba(15, 23, 42, 0.8)", backdropFilter: "blur(5px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }} onMouseDown={(e) => { if (e.target === e.currentTarget) setCheckoutUrl(null); }}>
          <div style={{ backgroundColor: "#0b4b37", width: "94%", maxWidth: "820px", height: "90vh", maxHeight: "840px", borderRadius: "16px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.2)", boxShadow: "0 25px 60px -15px rgba(0,0,0,0.8)", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.85rem 1.25rem", background: "#0b3324", borderBottom: "1px solid rgba(255,255,255,0.1)", color: "#fff" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                <CreditCard size={19} color="#d6f35c" />
