    ...
    <>
      ...
      <div className="platform-businesses">
        {businesses.map((business) => {
          ...
          const suspendConfirm = isSuspended ? ("Reactivar " + business.name + "?") : ("Suspender " + business.name + "?");
          const suspendTitle = isSuspended ? "Reactivar negocio" : "Suspender negocio";
          const lifetimeConfirm = business.isLifetime ? ("Quitar la membresía vitalicia de " + business.name + "?") : ("Dar membresía vitalicia a " + business.name + "?");
          const storeUrl = "https://" + business.slug + ".tupedido360.co";
          ...
          return (
            <article key={business.id}>
              ...
              <div className="platform-business-actions">
                ...
                <button disabled={busy === business.id} onClick={() => action(business, "toggleLifetime", lifetimeConfirm)} title={business.isLifetime ? "Quitar membresía vitalicia" : "Dar membresía vitalicia"}>
                  <InfinityIcon size={17} />
                </button>
                <button disabled={busy === business.id} onClick={() => action(business, "extendTrial", "Extender 30 días la prueba de " + business.name + "?")} title="Extender prueba 30 días">
                  <Clock3 size={17} />
                </button>
                <button disabled={busy === business.id} onClick={() => action(business, "toggleSuspended", suspendConfirm)} title={suspendTitle}>
                  {isSuspended ? <PlayCircle size={17} /> : <PauseCircle size={17} />}
                </button>
                <a href={storeUrl} target="_blank" rel="noreferrer" title="Abrir menú público">
