...
async function context() {
  ...
  return { businessId: session.businessId, userId: session.userId, userName: session.name, role: session.role, sql: await ensureSchema() };
}
...
export async function GET(request:Request) {
  ...
  if(new URL(request.url).searchParams.get("view")==="deleted"){
    if(auth.role!=="owner")return NextResponse.json({error:"Solo el dueño puede revisar pedidos eliminados."},{status:403});
    const deletedOrders=await auth.sql`SELECT id,order_reference AS reference,reason,deleted_by_name AS "deletedByName",deleted_by_role AS "deletedByRole",deleted_at AS "deletedAt",order_snapshot AS snapshot FROM order_deletion_log WHERE business_id=${auth.businessId} AND tenant_purged_at IS NULL ORDER BY deleted_at DESC LIMIT 100`;
...
export async function POST(request: Request) {
  const auth = await context();
  if (!auth) return NextResponse.json({ error: "Sesión no autorizada." }, { status: 401 });
  const body = await request.json().catch(() => null) as { action?: string; id?: string; status?: string; feeCop?: number; estimatedMinutes?: number; reason?: string } | null;
  ...
  if (body.action === "updatePaymentStatus" && body.status && paymentStatuses.has(body.status)) {
    if (!auth.role || !["owner", "admin", "cashier"].includes(auth.role)) return NextResponse.json({ error: "No tienes permiso para registrar pagos." }, { status: 403 });
    const [order] = await auth.sql`UPDATE orders SET payment_status=${body.status},paid=${body.status === "verified"},updated_at=now() WHERE id=${body.id} AND business_id=${auth.businessId} AND deleted_at IS NULL RETURNING id,payment_status AS "paymentStatus",paid`;
    if (!order) return NextResponse.json({ error: "Pedido no encontrado." }, { status: 404 });
    return NextResponse.json({ order });
  }
  ...
  if (body.action === "confirmDelivery") {
    ...
    const [order] = await auth.sql`UPDATE orders SET delivery_quote_status='confirmed',status='accepted',updated_at=now() WHERE id=${body.id} AND business_id=${auth.businessId} AND deleted_at IS NULL AND order_type='delivery' AND delivery_fee_cop IS NOT NULL RETURNING id`;
    if (!order) return NextResponse.json({ error: "Primero debes cotizar el domicilio." }, { status: 409 });
    return NextResponse.json({ order });
  }
  if (body.action === "deleteOrder") {
    if (!auth.role || !["owner","admin","cashier","kitchen"].includes(auth.role) || !auth.userId) return NextResponse.json({ error: "No tienes permiso para eliminar pedidos." }, { status: 403 });
    const reason=body.reason?.trim().slice(0,300)??"";if(reason.length<5)return NextResponse.json({error:"Escribe el motivo de la eliminación (mínimo 5 caracteres)."},{status:400});
    const actorId=auth.userId;const actorRole=auth.role;const orderId=body.id;
    const deleted=await auth.sql.begin(async transaction=>{const[order]=await transaction`SELECT * FROM orders WHERE id=${orderId} AND business_id=${auth.businessId} AND deleted_at IS NULL FOR UPDATE`;if(!order)return null;const items=await transaction`SELECT product_name AS "productName",unit_price_cop AS "unitPriceCop",quantity,subtotal_cop AS "subtotalCop" FROM order_items WHERE order_id=${order.id}`;await transaction`INSERT INTO order_deletion_log(order_id,business_id,order_reference,reason,deleted_by_user_id,deleted_by_name,deleted_by_role,order_snapshot) VALUES(${order.id},${auth.businessId},${order.reference},${reason},${actorId},${auth.userName},${actorRole},${transaction.json({...order,items})})`;await transaction`UPDATE orders SET deleted_at=now(),deleted_by_user_id=${actorId},deletion_reason=${reason},updated_at=now() WHERE id=${order.id}`;return order});
    if(!deleted)return NextResponse.json({error:"Pedido no encontrado o ya eliminado."},{status:404});return NextResponse.json({ok:true});
  }
  if(body.action==="purgeDeleted"){
    if(auth.role!=="owner")return NextResponse.json({error:"Solo el dueño puede retirar este registro de su negocio."},{status:403});
    const[log]=await auth.sql`UPDATE order_deletion_log SET tenant_purged_at=now() WHERE id=${body.id} AND business_id=${auth.businessId} AND tenant_purged_at IS NULL RETURNING id`;
    if(!log)return NextResponse.json({error:"Registro no encontrado."},{status:404});return NextResponse.json({ok:true});
  }
