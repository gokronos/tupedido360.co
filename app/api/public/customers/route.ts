...
export async function GET(request:Request){
  const slug=new URL(request.url).searchParams.get("slug")?.trim().toLowerCase();const session=slug?await currentCustomer(slug):null;
...
export async function POST(request:Request){
  ...
  const sql=await ensureSchema();const[order]=await sql`UPDATE orders SET delivery_quote_status='confirmed',status='accepted',updated_at=now() WHERE reference=${body.reference} AND customer_id=${session.customerId} AND business_id=${session.businessId} AND deleted_at IS NULL AND order_type='delivery' AND delivery_fee_cop IS NOT NULL AND delivery_quote_status='quoted' AND status NOT IN ('delivered','cancelled') RETURNING reference,status,total_cop AS "totalCop"`;
