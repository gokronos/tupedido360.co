...
export default async function StorePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [business] = await sql`SELECT id,name,slug,description,logo_url AS "logoUrl",logo_size AS "logoSize",primary_color AS "primaryColor",accent_color AS "accentColor",address,public_phone AS "publicPhone",whatsapp,menu_template AS "menuTemplate" FROM businesses WHERE slug=${slug} AND status IN ('trial','active')`;
