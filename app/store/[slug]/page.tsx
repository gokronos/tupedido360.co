...
export default async function StorePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [business] = await sql`SELECT id,name,slug,description,logo_url AS "logoUrl",primary_color AS "primaryColor",accent_color AS "accentColor",address,public_phone AS "publicPhone",whatsapp,menu_template AS "menuTemplate" FROM businesses WHERE slug=${slug} AND status IN ('trial','active')`;
  ...
  return <PublicStore business={{name:String(business.name),slug:String(business.slug),description:String(business.description),logoUrl:String(business.logoUrl),primaryColor:String(business.primaryColor),accentColor:String(business.accentColor),address:String(business.address),publicPhone:String(business.publicPhone),whatsapp:String(business.whatsapp),menuTemplate:String(business.menuTemplate)}} banners={banners.map(row=>({id:String(row.id),eyebrow:String(row.eyebrow),title:String(row.title),description:String(row.description),imageUrl:String(row.imageUrl)}))} categories={categories.map((row) => ({ id: String(row.id), name: String(row.name) }))} products={products.map((row) => ({ id: String(row.id), categoryId: row.categoryId ? String(row.categoryId) : null, name: String(row.name), description: String(row.description), priceCop: Number(row.priceCop), packagingFeeCop: Number(row.packagingFeeCop), icon: String(row.icon), imageUrl: String(row.imageUrl) }))} />;
}
