...
export default async function StorePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const products = await sql`
    SELECT id, category_id AS "categoryId", name, description, price_cop AS "priceCop", packaging_fee_cop AS "packagingFeeCop", icon, image_url AS "imageUrl", stock_quantity AS "stockQuantity"
    FROM products WHERE business_id=${business.id} AND active=true AND (stock_quantity IS NULL OR stock_quantity > 0) ORDER BY created_at DESC`;
