...
export async function currentCustomer(slug?:string){const jar=await cookies();const scoped=readCustomerToken(jar.get(slug?`${name}_${slug}`:name)?.value);if(scoped)return scoped;const legacy=slug?readCustomerToken(jar.get(name)?.value):null;return legacy?.businessSlug===slug?legacy:null}
export function customerCookie(slug:string){return{name:`${name}_${slug}`,options:{httpOnly:true,sameSite:"lax" as const,secure:process.env.NODE_ENV==="production",path:"/",maxAge:60*60*24*180,...(process.env.NODE_ENV==="production"?{domain:".tupedido360.co"}:{})}}}
